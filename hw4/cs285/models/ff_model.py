...
class FFModel(BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        if len(obs.shape)>1:
            ...
            actions = acs [None]
        feed_dict = self.data_feed(data_statistics)
        feed_dict.update({self.obs_pl: observations, self.acs_pl: actions})
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        # train the model
        feed_dict = self.data_feed(data_statistics)
        feed_dict.update({self.obs_pl: observations, self.acs_pl: actions, self.delta_labels: next_observations - observations})
