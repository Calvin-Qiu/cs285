...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        """
        return np.array([self.hist[self.preprocessor(s)] for s in states])
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        kl1 = tfp.distributions.kl_divergence(self.encoder1, self.prior)
        kl2 = tfp.distributions.kl_divergence(self.encoder2, self.prior)
        self.kl = kl1 + kl2
    ...
    def update(self, state1, state2, target):
        ...
        feed = {self.state1: state1, self.state2: state2, self.discrim_target: target}
    ...
    def get_likelihood(self, state1, state2):
        ...
        discrim_target = np.ones((state1.shape[0], 1))
        likelihood = self.sess.run(self.likelihood, feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: discrim_target})
        return likelihood
